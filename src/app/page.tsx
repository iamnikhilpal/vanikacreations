          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 p-4 sm:p-6">
            <div className="relative h-48 sm:h-72 md:h-96 group">
              <div className="relative w-full h-full bg-[#F5EFE6] rounded-lg">
                <Image src={images[currentImageIndex]} alt="Diwali gift hamper with sweets, dry fruits and chocolates" fill className="object-contain transition-opacity duration-300" sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, 33vw" />
              </div>
            </div>
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#F5EFE6]">
      {/* Navigation Bar */}
      <nav
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#F5EFE6]/90 backdrop-blur-xl border-b border-[#E1B07E]/20' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-3 sm:px-6 w-full flex items-center justify-between py-3 sm:py-4">
          <div className="flex items-center space-x-4">
            <Link href="/products" className="flex items-center">
              <div className="w-16 h-8 sm:w-24 sm:h-10 md:w-32 md:h-12 relative flex items-center justify-center">
                <Image src="/vanika.png" alt="Vanika Creations logo" fill className="object-contain max-w-full max-h-full" priority />
              </div>
            </Link>
          ...
          <div className="hidden md:flex space-x-8">
            <button
              onClick={() => document.querySelector('#home-banner')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
              className="capitalize transition-colors duration-200 text-[#2C2C2C] hover:text-[#B08BBB]">
              Home
            </button>
            <button
              onClick={() => document.querySelector('#product-grid')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
              className="capitalize transition-colors duration-200 text-[#2C2C2C] hover:text-[#B08BBB]">
              Products
            </button>
            <button
              onClick={() => document.querySelector('#contact-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
              className="capitalize transition-colors duration-200 text-[#2C2C2C] hover:text-[#B08BBB]">
              Contact
            </button>
          </div>
      ...
      {/* Banner Section */}
      <section id="home-banner" className="pt-32 pb-20 relative min-h-[80vh] flex items-center">
        <div className="absolute inset-0 w-full h-full">
          <Image
            src="/banner.png"
            alt="Diwali gift hamper with sweets, dry fruits and chocolates"
            fill
            sizes="100vw"
            quality={100}
            priority
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 w-full">
          <div className="text-center">
            <h1 className="text-2xl sm:text-4xl md:text-6xl font-bold mb-4 text-[#9A3324] drop-shadow-sm">Diwali Hampers by Vanika Creations</h1>
            <h2 className="text-lg sm:text-2xl md:text-3xl font-semibold mb-6 text-[#B08BBB]">Exquisite Gift Hampers for the Festival of Lights</h2>
            ...
            <div className="bg-[#F5EFE6]/50 p-6 rounded-lg backdrop-blur-sm max-w-3xl mx-auto border border-[#E1B07E]/30">
              <p className="text-lg md:text-xl text-[#2C2C2C] leading-relaxed mb-4">
                Discover our carefully curated collection of luxury Diwali hampers, featuring artisanal sweets, premium dry fruits, and handcrafted decoratives.
              </p>
              <p className="text-base md:text-lg text-[#2C2C2C]/90 leading-relaxed">
                Each hamper is thoughtfully designed to bring joy and celebration to your loved ones. Pre-order now for special early bird discounts!
              </p>
            </div>
            ...
            <button
              ...
              <svg className="w-5 h-5 transform transition-transform group-hover:translate-y-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section id="product-grid" className="py-20 relative scroll-mt-24">
        <div className="max-w-6xl mx-auto px-6 mb-6">
          <h2 className="text-2xl font-bold text-[#B08BBB]">Shop by Category</h2>
          <div className="flex gap-4 mt-3 flex-wrap">
            <button className="px-3 py-1 rounded-full bg-[#F5EFE6] border border-[#E1B07E]/30 text-[#2C2C2C]">Luxury Hampers</button>
            <button className="px-3 py-1 rounded-full bg-[#F5EFE6] border border-[#E1B07E]/30 text-[#2C2C2C]">Family Hampers</button>
            <button className="px-3 py-1 rounded-full bg-[#F5EFE6] border border-[#E1B07E]/30 text-[#2C2C2C]">Dry Fruits</button>
            <button className="px-3 py-1 rounded-full bg-[#F5EFE6] border border-[#E1B07E]/30 text-[#2C2C2C]">Sweets & Chocolates</button>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 items-stretch">
            {products.map((product) => {
              ...
              return (
                <div key={product.id} className="bg-white rounded-2xl overflow-hidden border border-[#E1B07E]/30 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col h-full group min-h-[400px] sm:min-h-[500px]">
                  ...
                  <div className="p-8 flex flex-col flex-grow space-y-4 justify-between">
                    ...
                    <p className="text-[#2C2C2C] leading-relaxed text-base min-h-[4.5rem] line-clamp-3">{shortDesc}</p>

                    <div className="pt-4 space-y-4 mt-auto">
                      <p className="text-xl font-semibold text-[#9A3324] border-t border-[#E1B07E]/30 pt-4">{product.priceRange}</p>
                      <button onClick={() => handleOpenModal(product)} className="w-full bg-[#B08BBB] hover:bg-[#9A3324] text-white px-8 py-4 rounded-full font-semibold text-lg transition-all duration-300 transform hover:scale-102 hover:shadow-lg flex items-center justify-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              <div className="flex items-center gap-3 justify-center w-full">
                ...
                <div>
                  ...
                  <a href="mailto:vanikacreations2025@gmail.com" className="text-[#2C2C2C]/80 hover:text-[#B08BBB] transition-colors break-all">vanikacreations2025@gmail.com</a>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 justify-center w-full -ml-11">
                ...
                <div>
                  ...
                  <a href="tel:+917116299557" className="text-[#2C2C2C]/80 hover:text-[#B08BBB] transition-colors">+91 (7116) 299-557</a>
                </div>
              </div>

              <div className="flex items-center gap-3 justify-center w-full">
                <a href="https://www.instagram.com/vanika_creations_/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 bg-[#B08BBB] hover:bg-[#9A3324] text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-md w-full sm:w-auto">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M7.75 2h8.5A5.75 5.75 0 0 1 22 7.75v8.5A5.75 5.75 0 0 1 16.25 22h-8.5A5.75 5.75 0 0 1 2 16.25v-8.5A5.75 5.75 0 0 1 7.75 2zm0 1.5A4.25 4.25 0 0 0 3.5 7.75v8.5A4.25 4.25 0 0 0 7.75 20.5h8.5A4.25 4.25 0 0 0 20.5 16.25v-8.5A4.25 4.25 0 0 0 16.25 3.5h-8.5zm4.25 3.25a5.25 5.25 0 1 1 0 10.5a5.25 5.25 0 0 1 0-10.5zm0 1.5a3.75 3.75 0 1 0 0 7.5a3.75 3.75 0 0 0 0-7.5zm5.25.75a1 1 0 1 1-2 0a1 1 0 0 1 2 0z" /></svg>
      ...
      <div className="text-center p-8 bg-[#F5EFE6] border-t border-[#E1B07E]/20">
        <a href={`${process.env.BUCKET_URL}/vanika-diwali-hampers.pdf`} download className="inline-flex items-center space-x-2 bg-[#B08BBB] hover:bg-[#9A3324] text-white px-6 py-3 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-md hover:shadow-lg">
